...
# aliases kept for test_den_mode.py (same canned turn JSON, den flow)
RAYHAN_STATE = RAYHAN_DONE
MAYA_STATE = MAYA_DONE
...
async def test_trigger_uses_group_membership_and_snapshots_first():
    ...
    # trigger phrase never leaks into the occasion (it would echo in asks)
    assert active.session.occasion == "dinner this weekend?"
...
async def test_all_assents_lock_confirm_and_snapshot_dms():
    ...
    # the chat transforms around the plan: confetti + rename + tappable invite
    assert [c for c, *_ in deps["messaging"].celebrations] == ["g1"]
    assert all(name for _, _, name, _ in deps["messaging"].celebrations)  # rename in group mode
    assert [c for c, _ in deps["messaging"].files] == ["g1"]
